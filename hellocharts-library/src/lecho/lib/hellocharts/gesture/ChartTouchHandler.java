...
//        Log.e("ChartApp",event.toString());
    ...
    protected class ChartGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            if (isScrollEnabled) {
//                showChartLog(distanceX+" <onScrolldistanceX");
                if(distanceX>0&&computator.isFlyEnd())
                {
                    return false;
                }
                distanceX=Math.min(distanceX,computator.getLastRight()
                        -computator.getContentRectMinusAllMargins().right+10);
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            showChartLog("飞起来x:"+velocityX);
            if (isScrollEnabled) {
                if(velocityX<0&&computator.isFlyEnd())
                    return false;
