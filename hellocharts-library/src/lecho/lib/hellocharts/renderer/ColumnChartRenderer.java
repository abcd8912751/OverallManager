...
public class ColumnChartRenderer extends AbstractChartRenderer {
    ...
    public ColumnChartRenderer(Context context, Chart chart, ColumnChartDataProvider dataProvider) {
        ...
        columnSpacing=subcolumnSpacing*100;
    ...
    private void drawColumnsForSubcolumns(Canvas canvas) {
        ...
        for (Column column : data.getColumns()) {
            ...
            float columnWidth=baseColumnWidth *subColumnSize+(subcolumnSpacing * (subColumnSize - 1));
...
//        processColumnForSubcolumns(canvas, column, columnWidth, selectedValue.getFirstIndex(), MODE_HIGHLIGHT);
    ...
    private void processColumnForSubcolumns(Canvas canvas, Column column, float columnWidth, int columnIndex,
        ...
        List<SubcolumnValue> columnValues=column.getValues();
        int size=columnValues.size(),valueIndex = 0,
                columnSize=dataProvider.getColumnChartData().getColumns().size();
        ...
        if(columnIndex>0)
            rawX=computator.getTempRight()+halfColumnWidth+columnSpacing;
        else
        {
            if(computator.getFirstLeft()==0)
            {
                rawX=computator.getContentRectMinusAllMargins().left
                        +halfColumnWidth+10;
            }
            computator.setFirstLeft(rawX-halfColumnWidth);
        }
        float subcolumnRawX = rawX - halfColumnWidth;
        computator.setTempRight(rawX+halfColumnWidth);
        computator.setColumnRawX(columnIndex,rawX);
        if(columnIndex==columnSize-1)
            computator.setLastRight(rawX+halfColumnWidth);
        for (SubcolumnValue columnValue : column.getValues())
        {
...
//        float left=computator.computeRawX()
    ...
    public void setLabelFontSize(float fontSize)
    {
        ...
        int defaultSize=ChartUtils.sp2px(scaledDensity, data.getValueLabelTextSize());
        labelPaint.setTextSize(Math.max(defaultSize,fontSize));
    }
}
