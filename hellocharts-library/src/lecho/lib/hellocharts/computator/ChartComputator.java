...
import static lecho.lib.hellocharts.util.ChartUtils.showChartLog;
...
public class ChartComputator {
    ...
    protected float tempRight=0,lastRight=0,firstLeft=0;
    protected float[] tempRawX;
    /**
...
//        showChartLog(maxViewport.right+"<max-right>"+right);
    ...
    public void setViewportTopLeft(float left, float top) {
         ...
         * would be 0 to 8.
         */
        final float curLeft = currentViewport.left;
        final float curWidth = currentViewport.width();
        final float curHeight = currentViewport.height();
        float width2=maxViewport.right - curWidth;
        float beforeLeft = left;
        float distance = lastRight-contentRectMinusAllMargins.right+30;
        float distanceOffset =distance*curWidth/contentRectMinusAllMargins.width();
//        left = Math.max(maxViewport.left, Math.min(left, maxViewport.right - curWidth));
        left=Math.max(maxViewport.left,Math.min(left,curLeft+distanceOffset));   //修改后
        showChartLog("lastRight:"+getLastRight()+",curLeft:"+curLeft+",left:"+(left)+",distanceOffset"+distanceOffset+",curWidthPersent:"+curWidth/contentRectMinusAllMargins.width());
        top = Math.max(maxViewport.bottom + curHeight, Math.min(top, maxViewport.top));
        if(left-curLeft<0||!isFlyEnd())
    ...
    public float getTempRight() {
        return tempRight;
    }

    public void setTempRight(float tempRight) {
    ...
    public float[] getTempRawX() {
        return tempRawX;
    }

    public void setTempRawX(float[] tempRawX) {
        this.tempRawX = tempRawX;
    }
    ...
    public void resetTempRawX(int size)
    {
        this.tempRawX=new float[size];
    }

    public void setColumnRawX(int columnIndex, float rawX) {
        try {
            if(rawX==-111)
                rawX+=0.1;
            this.tempRawX[columnIndex]=rawX;
        } catch (Exception e) {
        }
    }
    public float getColumnRawx(int columnIndex)
    {
        try {
            return tempRawX[columnIndex];
        } catch (Exception e) {
            showChartLog("I'm wrong over");
            return -111;    //设定为不会触碰到的值
        }
    }
    ...
    public boolean isFlyEnd()
    {
        ...
        {
            if(lastRight<getContentRectMinusAllMargins().right)
            {
                showChartLog("飘到最后了");
                return true;
            }
